...
//Добавляем текущему датуму информацию о том какой датум на него ссылается, дабы в дальнейшем почистить эту ссылку
/datum/proc/referenced_in(datum/D, variable = null)
	if(!reference_in)
		reference_in = list()
	reference_in[D] = variable

/datum/proc/remove_referenced(datum/D)
	if(!reference_in || !reference_in.len)
		return 1

	reference_in.Remove(D)

/datum/proc/clear_references()
	if(!reference_in || !reference_in.len)
		return 1

	for(var/datum/D in reference_in)
		var/reference_variable = reference_in[D]
//		world << reference_variable
		D.vars[reference_variable] = null
		reference_in.Remove(D)