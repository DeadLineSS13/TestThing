...
/obj/item/weapon/storage
	...
	var/max_weight = 30
	var/wet_protected = 0
	var/back_open = 0	//Можем ли открывать рюкзак, висящий на спине

/obj/item/weapon/storage/get_weight(with_alpha = 0)
	var/weight_sum = weight
	var/alpha = 1
	if(with_alpha)
		alpha = weight_alpha
	for(var/obj/item/I in contents)
		weight_sum += I.get_weight() * alpha
	return weight_sum

/obj/item/weapon/storage/get_wet()
	if(!wet_protected)
		for(var/obj/item/I in contents)
			I.get_wet()
...
/obj/item/weapon/storage/MouseDrop(atom/over_object)
	if(iscarbon(usr)) //all the check for item manipulation are in other places, you can safely open any storages as anything and its not buggy, i checked
		...
		if(over_object == M && loc != M && Adjacent(M)) // this must come before the screen objects only block
...
/obj/item/weapon/storage/throw_at(atom/target, range, speed, mob/thrower, spin, diagonals_first = 0, speed_sleep = 1)
	close_all()
	return ..()
...
/obj/item/weapon/storage/proc/standard_orient_objs(rows, cols, list/obj/item/display_contents)
	var/cx = 4
	var/cy = rows
	boxes.screen_loc = "WEST+4:17,SOUTH:25 to WEST+[4+cols]:17,SOUTH+[rows]:25"

	if(display_contents_with_number)
		for(var/datum/numbered_display/ND in display_contents)
			ND.sample_object.mouse_opacity = 2
			ND.sample_object.screen_loc = "WEST+[cx]:17,SOUTH+[cy]:25"
		...
		for(var/obj/O in contents)
			...
			O.screen_loc = "WEST+[cx]:17,SOUTH+[cy]:25"
			if(!istype(O, /obj/item))
//				var/obj/item/I = O
//				if(I.stackable)
//					I.maptext = "<font color='white'>[I.current_stack-1 ? I.current_stack : ""]</font>"
//				else
//					I.maptext = ""
//			else
				O.maptext = ""
			...
			if(cx > (4+cols))
				cx = 4
				cy--
	closer.screen_loc = "WEST+[4+cols+1]:17,SOUTH:25"
...
/obj/item/weapon/storage/proc/can_be_inserted(obj/item/W, stop_messages = 0, mob/user)
	...
	var/sum_weight_class = W.get_weight()
	for(var/obj/item/I in contents)
		sum_weight_class += I.get_weight()


	if((sum_w_class > max_combined_w_class) || (sum_weight_class > max_weight))
		if(!stop_messages)
...
/obj/item/weapon/storage/proc/handle_item_insertion(obj/item/W, prevent_warning = 0)
	...
	if(usr)
		...
		for(var/obj/item/I in contents - W)
			if(I.type == W.type)
				if(I.stackable && I.stackable > I.current_stack)
					I.attackby(W, usr)
					update_icon()
					return 1
...
/obj/item/weapon/storage/proc/remove_from_storage(obj/item/W, atom/new_location, burn = 0)
	...
	if(W.maptext)
//		if(W.stackable)
//			W.maptext = "<font color='white'>[W.current_stack-1 ? W.current_stack : ""]</font>"
//		else
		W.maptext = ""
...
/obj/item/weapon/storage/attackby(obj/item/W, mob/user, params)
	...
	playsound(loc, insert_sound, 50, 1, -5, channel = "regular", time = 5)
	return 1
...
/obj/item/weapon/storage/attack_hand(mob/user)
	if(iscarbon(user))
		...
		if(C.back == src && !back_open)
			user << "<span class='warning'>Ты не можешь дотянуться до рюкзака на спине! Возьми его в руки!</span>"
			return
...
/obj/item/weapon/storage/Initialize()
	...
	boxes.screen_loc = "WEST+4:17,SOUTH:25 to WEST+4:17,SOUTH:24"
	boxes.layer = HUD_LAYER
	boxes.plane = HUD_PLANE
	boxes.referenced_in(src, "boxes")
	closer = new /obj/screen/close()
	closer.master = src
	closer.icon_state = "backpack_close"
	closer.layer = ABOVE_HUD_LAYER
	closer.plane = ABOVE_HUD_PLANE
	closer.referenced_in(src, "closer")
	orient2hud()
	..()
...
/obj/item/weapon/storage/equipped(mob/user)
	if(!back_open)
		close(user)
