...
/obj/item/weapon/restraints/handcuffs/attack(mob/living/carbon/C, mob/living/carbon/human/user)
	...
	if(!C.handcuffed)
		...
		playsound(loc, cuffsound, 30, 1, -2, channel = "regular", time = 5)
		if(do_mob(user, C, 30))
			apply_cuffs(C,user)
...
/obj/item/weapon/restraints/legcuffs/beartrap/suicide_act(mob/user)
	...
	playsound(loc, 'sound/weapons/bladeslice.ogg', 50, 1, -1, channel = "regular", time = 10)
	return (BRUTELOSS)
	...
	if(armed && isturf(src.loc))
		if(isliving(AM))
			...
			if(snap)
				armed = 0
				icon_state = "[initial(icon_state)][armed]"
				playsound(src.loc, 'sound/effects/snap.ogg', 50, 1, channel = "regular", time = 5)
