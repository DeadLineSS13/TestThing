...
//		M.IgniteMob()
	...
	if(smoketime < 1)
		...
		if(ismob(loc))
			...
			M.unEquip(src, 1, 0)	//un-equip it so the overlays can update //Force the un-equip so the overlays update
		qdel(src)
		return
	if(location)
		location.hotspot_expose(700, 5)
	...
	if(user.r_hand == src || user.l_hand == src)
		if(!lit)
			...
			if(!istype(src, /obj/item/weapon/lighter/greyscale))
				...
				playsound(user, 'sound/stalker/objects/zippo_open.ogg', 100, 1, channel = "regular", time = 5)
			else
				if(prob(75))
				...
				playsound(user, 'sound/stalker/objects/lighter_scratch.ogg', 100, 1, channel = "regular", time = 5)
...
/obj/item/weapon/rollingpaper/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/weapon/reagent_containers/food/snacks/grown))
		...
		if(O.dry)
			user.unEquip(target, 1, 0)
			user.unEquip(src, 1, 0)
...
/obj/item/weapon/storage/box/matches
	name = "matchbox"
	desc = "Маленькая коробка со спичками. Если верить надписи, вмещает в себя 40 спичек."
	icon = 'icons/obj/cigarettes.dmi'
	icon_state = "matchbox"
	item_state = "zippo"
	storage_slots = 10
...
/obj/item/weapon/storage/box/matches/attackby(obj/item/weapon/match/W as obj, mob/user as mob, params)
	if(istype(W, /obj/item/weapon/match))
		playsound(user, 'sound/stalker/objects/match_ignite.ogg', 100, 1, channel = "regular", time = 5)
		W.matchignite()
		//user.add_light_range(1)
	return
