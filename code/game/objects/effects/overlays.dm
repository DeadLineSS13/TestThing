...
/obj/effect/overlay/temp/impact_effect/New(loc, atom/target)
//	if(target == P.original) //the projectile hit the target originally clicked
//		pixel_x = P.p_x + target.pixel_x - 16 + rand(-5,5)
//		pixel_y = P.p_y + target.pixel_y - 16 + rand(-5,5)
//	else
	pixel_x = target.pixel_x + rand(-5,5)
	pixel_y = target.pixel_y + rand(-5,5)
	..()
