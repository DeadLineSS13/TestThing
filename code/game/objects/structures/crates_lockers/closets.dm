/obj/structure/closet
	...
	var/storage_capacity = 500 //This is so that someone can't pack hundreds of items in a locker/crate then open it in a populated area to crash clients.
...
/obj/structure/closet/proc/open()
	if(opened || !can_open())
		return 0
	opened = 1
	playsound(loc, open_sound, 15, 1, -3, channel = "regular", time = 5)
	density = 0
	dump_contents()
	update_icon()
	return 1
	...
	if(opened)
		if(istype(W, /obj/item/weapon/grab))
			if(large)
				...
				MouseDrop_T(G.owner.pulling, user)	//act like they were dragged onto the closet
				user.drop_item()
			else
		...
		if(istype(W, cutting_tool))
			if(istype(cutting_tool, /obj/item/weapon/weldingtool))
				...
				playsound(loc, cutting_sound, 40, 1, channel = "regular", time = 5)
				if(do_after(user,40/W.toolspeed,1, target = src))
					if( !opened || !istype(src, /obj/structure/closet) || !user || !WT || !WT.isOn() || !user.loc )
						return
					playsound(loc, cutting_sound, 50, 1, channel = "regular", time = 5)
					new material_drop(loc)
