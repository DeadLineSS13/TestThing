...
/obj/structure/mirror/proc/shatter()
	...
	playsound(src, "shatter", 70, 1, channel = "regular", time = 10)
	desc = "Oh no, seven years of bad luck!"


/obj/structure/mirror/bullet_act(obj/item/projectile/Proj)
	if(prob(Proj.damage_add * 2))
		if((Proj.damage_type == BRUTE || Proj.damage_type == BURN))
			if(!shattered)
				shatter()
			else
				playsound(src, 'sound/effects/hit_on_shattered_glass.ogg', 70, 1, channel = "regular", time = 5)
	..()
...
/obj/structure/mirror/attackby(obj/item/I, mob/living/user, params)
		...
		if(istype(I, /obj/item/weapon/weldingtool))
			...
			if(WT.remove_fuel(0, user))
				...
				if(do_after(user, 10/I.toolspeed, target = src))
					...
					desc = initial(desc)
				return
		playsound(src.loc, 'sound/effects/hit_on_shattered_glass.ogg', 70, 1, channel = "regular", time = 5)
		return
	...
	if(prob(I.force * 2))
		...
		playsound(src.loc, 'sound/effects/Glasshit.ogg', 70, 1, channel = "regular", time = 5)
