...
/obj/item/Initialize()
	for(var/path in actions_types)
		new path(src)
	if(!icon_ground && icon_states(icon).Find("[icon_state]_ground"))
...
/obj/item/examine(mob/user) //This might be spammy. Remove?
	...
	user << user.client.select_lang("[size] �������.","[pronoun] a [size] item.") //e.g. They are a small item. or It is a bulky item.

	var/real_weight = round(get_weight(), 0.01)
	if(weight <= 0)
		user << user.client.select_lang("��������� ���������� ���������.","Feels completely weightless.")
		return

	var/ending = "a"
	var/need_ending = 0
	var/nfe = round(real_weight)		//num for examine
	var/w_word
	var/w_word_en
	if(real_weight)
		if(real_weight <= 0.1)
	...
	if(nfe)
		...
		if(w_word)
			if(need_ending)
				user << user.client.select_lang("����� [w_word] [nfe] ���������[ending].","Its weight is [w_word_en] [nfe]kg.")
			else
				user << user.client.select_lang("����� [nfe] [w_word] ���������[ending].","Its weight is [w_word_en] [nfe]kg.")
		else
			user << user.client.select_lang("����� [nfe] ���������[ending].","Its weight is [nfe]kg.")
	else if(w_word)
		user << user.client.select_lang("����� [w_word] ���������[ending].","Its weight is [w_word_en] kg.")
	else
		user << user.client.select_lang("����� INFINITY ���������[ending].","Its weight is INFINITY kg.")					//����� ����� ���� �� ������


	if(iscarbon(user))
		var/mob/living/carbon/C = user
		var/datum/data/record/sk = (find_record("sid", C.sid, GLOB.data_core.stalkers))
...
/obj/item/proc/show_count()
//	maptext = "<font color='white'>[current_stack-1 ? current_stack : ""]</font>"
	update_icon()

/obj/item/proc/remove_count()
	maptext = ""
	update_icon()
	...
	if(src in user.held_items)
		if(stackable)
			if(!(istype(loc, /obj/item/weapon/storage)))
				if(current_stack > 1)
					current_stack--
					var/obj/item/N = new type(loc)
					user << "<spawn class='notice'>You've took one [src], now it has [current_stack].</span>"
					weight -= N.weight
					show_count()
					N.attack_hand(user)
					return
*/
	...
	if (istype(src.loc, /obj/item/weapon/storage))
		...
		if (!istype(src, /obj/item/weapon/gun/projectile))
			...
			user.visible_message("<span class='danger'>[user] starts to take out the weapon...</span>", "<span class='notice'>You start to take out [G]...</span>", "<span class='danger'>[user] ������� �� �������...</span>", "<span class='notice'>�� �������� � [G.name_ru]...</span>")
			if(do_after_inventory(user, (G.weapon_weight * 15 + 5)*S.takeout_speed, 1, G))
				...
				S.remove_from_storage(src, user.loc)
				user.visible_message("<span class='danger'>[user] took out [G].</span>", "<span class='notice'>You took out the [G].</span>", "<span class='danger'>[user] ������� [G.name_ru].</span>", "<span class='notice'>�� �������� [G.name_ru].</span>")
	...
	if(stackable)
		...
		if(!I)
			if(current_stack > 1)
				current_stack--
				var/obj/item/N = new type(loc)
				if(istype(N, /obj/item/ammo_casing))
					var/obj/item/ammo_casing/AC = N
					var/obj/item/ammo_casing/Asrc = src
					if(!Asrc.BB)
						AC.BB = null
						AC.update_icon()
				user << "<spawn class='notice'>You've took one [src], now it has [current_stack].</span>"
				weight -= N.weight
				usr.put_in_active_hand(N)
				show_count()
				playsound(user, 'sound/stalker/weapons/load/ammo_manip.wav', 50, 1, channel = "regular", time = 5)
		else if(type == I.type && isturf(loc))
			if(I.current_stack < I.stackable)
				if(istype(src, /obj/item/ammo_casing))
					var/obj/item/ammo_casing/AC = src
					var/obj/item/ammo_casing/A = I
					if(A.BB && AC.BB && A.BB.type != AC.BB.type)
						return 0
					else if((A.BB && !AC.BB) || (!A.BB && AC.BB))
						return 0
				if(I.current_stack <= I.stackable - current_stack)
					I.current_stack += current_stack
					var/hand_index = user.get_held_index_of_item(src)
					if(hand_index)
						user.held_items[hand_index] = null
						user.update_inv_hands()
					user << "<span class = 'notice'>You've stacked [I], now it has [I.current_stack].</span>"
					I.weight += weight
					I.show_count()
					qdel(src)
				else
					var/diff = I.stackable - I.current_stack
					current_stack = current_stack - diff
					I.current_stack = I.stackable
					user << "<span class = 'notice'>You've stacked [I], now it has [I.current_stack].</span>"
					I.weight += initial(weight) * diff
					weight -= initial(weight) * diff
					I.show_count()
					show_count()
				playsound(user, 'sound/stalker/weapons/load/ammo_manip.wav', 50, 1, channel = "regular", time = 5)
		else if(I && type == I.type && I.stackable)
			if(istype(src, /obj/item/ammo_casing))
				var/obj/item/ammo_casing/AC = src
				var/obj/item/ammo_casing/A = I
				if(A.BB && AC.BB && A.BB.type != AC.BB.type)
	...
	if(stackable)
		if(W.type == type)
			...
			if(isturf(loc))
				RightClick(user)
/*
				if(W.current_stack < W.stackable)
					if(W.current_stack <= W.stackable - current_stack)
						W.current_stack += current_stack
						var/hand_index = user.get_held_index_of_item(W)
						if(hand_index)
							user.held_items[hand_index] = null
							user.update_inv_hands()
						user << "<span class = 'notice'>You've stacked [W], now it has [W.current_stack].</span>"
						W.weight += weight
						W.show_count()
						qdel(src)
					else
						var/diff = W.stackable - W.current_stack
						current_stack = current_stack - diff
						W.current_stack = W.stackable
						user << "<span class = 'notice'>You've stacked [W], now it has [W.current_stack].</span>"
						W.weight += initial(weight) * diff
						weight -= initial(weight) * diff
						W.show_count()
						show_count()
					playsound(user, 'sound/stalker/weapons/load/ammo_manip.wav', 50, 1, channel = "regular", time = 5)
*/
				...
				if(current_stack < stackable)
					if(current_stack <= stackable - W.current_stack)
						current_stack += W.current_stack
						var/hand_index = user.get_held_index_of_item(W)
						if(hand_index)
							user.held_items[hand_index] = null
							user.update_inv_hands()
						user << "<span class = 'notice'>You've stacked [src], now it has [current_stack].</span>"
						weight += W.weight
						show_count()
						qdel(W)
					else
						var/diff = stackable - current_stack
						W.current_stack = W.current_stack - diff
						current_stack = stackable
						user << "<span class = 'notice'>You've stacked [src], now it has [current_stack].</span>"
						weight += initial(W.weight) * diff
						W.weight -= initial(W.weight) * diff
						W.show_count()
						show_count()
					playsound(user, 'sound/stalker/weapons/load/ammo_manip.wav', 50, 1, channel = "regular", time = 5)
			return 1
...
/obj/item/proc/mob_can_equip(mob/M, slot, disable_warning = 0, delay = 1)
	if(!M)
		return 0

	var/can_equip = M.can_equip(src, slot, disable_warning)

	if(equip_delay && delay && can_equip)
		if(flags & IN_PROGRESS)
			return

		flags += IN_PROGRESS
		if(!do_after(M, equip_delay, 1, src))
			flags &= ~IN_PROGRESS
			return
		flags &= ~IN_PROGRESS
...
/obj/item/verb/verb_pickup()
	...
	if(usr.stat || usr.restrained() || !Adjacent(usr) || usr.stunned || usr.weakened)
		return
...
/obj/item/proc/eyestab(mob/living/carbon/M, mob/living/carbon/user)
	...
	if(ishuman(M))
		...
		if((H.head && H.head.flags_cover & HEADCOVERSEYES) || \
			...
			user << user.client.select_lang("<span class='danger'>���� �� ������� ������.</span>", "<span class='danger'>I have to remove eye protection first</span>")
			return

	src.add_fingerprint(user)

	playsound(loc, src.hitsound, 30, 1, -1, channel = "regular", time = 10)

	if(M != user)
		M.visible_message("<span class='danger'>[user] has stabbed [M] in the eye with [src]!</span>", \
							"<span class='userdanger'>[user] stabs you in the eye with [src]!</span>", \
							"<span class='danger'>[user] ������� [name_ru] � ���� [M]!</span>", \
							"<span class='userdanger'>[user] ������� ���� � ���� [name_ru]!</span>")
		user.do_attack_animation(M)
	else
		user.visible_message( \
			"<span class='danger'>[user] has stabbed themself in the eyes with [src].</span>", \
			"<span class='userdanger'>You stab yourself in the eyes with [src].</span>", \
			"<span class='danger'>[user] ������� ���� � ���� [name_ru].</span>", \
			"<span class='userdanger'>�� ������� ���� � ���� [name_ru]!</span>")
	if(is_human_victim)
	...
	if (M.eye_stat >= 10)
		...
		if(M.stat != 2)
			M << M.client.select_lang("<span class='danger'>��� ����� ����������</span>", "<span class='danger'>Your eyes start to bleed.</span>")
		if (!(M.disabilities & (NEARSIGHT | BLIND)))
			M.disabilities |= NEARSIGHT
			M << M.client.select_lang("<span class='danger'>�� ����������� �����������!</span>", "<span class='danger'>You become nearsighted!</span>")
		if(prob(50))
			if(M.stat != 2)
				if(M.drop_item())
					M << M.client.select_lang("<span class='danger'>�� ������� ��� ������ � ���������� �� ������.</span>", "<span class='danger'>You drop what you're holding and clutch at your eyes!</span>")
			M.eye_blurry += 10
			M.Paralyse(1)
			M.Weaken(2)
		if (prob(M.eye_stat - 10 + 1) && !(M.disabilities & BLIND))
			if(M.stat != 2)
				M << M.client.select_lang("<span class='danger'>� ��� � �������.</span>", "<span class='danger'>Now you're blind.</span>")
...
/obj/item/throw_at(atom/target, range, speed, mob/thrower, spin=1, diagonals_first = 0, speed_sleep = 1)
	thrownby = thrower
	. = ..()
...
/obj/item/proc/get_add_weight()
	return add_weight

/obj/item/proc/get_wet()
	return