...
/turf/stalker/Entered(atom/A, atom/OL)
	...
	if(istype(A, /mob/living))
		if(istype(src, /turf/stalker/floor/water))
			...
			playsound(src, footstepsound, 60, 1, channel = SSchannels.get_reserved_channel(rand(22,30)))
			..()
			return
	if(istype(A, /mob/living))
		var/mob/living/L = A
		...
		if(istype(M, /mob/living/carbon/human))
			...
			if(istype(H.shoes, /obj/item/clothing/shoes/clown_shoes))
				if(M.m_intent == "run")
					if(M.footstep >= 1)
						M.footstep = 0
						playsound(src, "clownstep", 30, 1, channel = SSchannels.get_reserved_channel(rand(22,30))) // this will get annoying very fast.
					else
						M.footstep++
				else
					playsound(src, "clownstep", 60, 1, channel = SSchannels.get_reserved_channel(rand(22,30)))
			...
			if(istype(H.shoes, /obj/item/clothing/shoes))
				if(M.m_intent == "run")
					if(M.footstep > 0)
						M.footstep = 0
						playsound(src, footstepsound, 20, 1, channel = SSchannels.get_reserved_channel(rand(22,30))) // this will get annoying very fast.
					else
						M.footstep++
				else
					//if(M.footstep >= 1)
						//M.footstep = 0
					playsound(src, footstepsound, 20, 1, channel = SSchannels.get_reserved_channel(rand(22,30)))
					//else
						//M.footstep++
