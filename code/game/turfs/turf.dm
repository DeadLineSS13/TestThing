...
/turf/handle_fall(mob/faller, forced)
	...
	if(!forced)
		return
	if(has_gravity(src))
		playsound(src, "bodyfall", 50, 1, channel = "regular", time = 5)
...
/turf/handle_slip(mob/living/carbon/C, s_amount, w_amount, obj/O, lube)
	if(has_gravity(src))
		...
		playsound(C.loc, 'sound/misc/slip.ogg', 50, 1, -3, channel = "regular", time = 5)
...
/turf/indestructible/splashscreen
	...
	icon = 'icons/stalker/images/lobby.dmi'
	layer = FLY_LAYER
