	...
	if(istype(C, /obj/item/stack/rods))
		...
		if (R.get_amount() < 2)
			...
			if(do_after(user, 30, target = src))
				if (R.get_amount() >= 2 && !istype(src, /turf/simulated/floor/engine))
					...
					playsound(src, 'sound/items/Deconstruct.ogg', 80, 1, channel = "regular", time = 10)
					R.use(2)
		...
		if( welder.isOn() && (broken || burnt) )
			if(welder.remove_fuel(0,user))
				...
				playsound(src, 'sound/items/Welder.ogg', 80, 1, channel = "regular", time = 10)
...
/turf/simulated/floor/engine/attackby(obj/item/weapon/C, mob/user, params)
	...
	if(istype(C, /obj/item/weapon/wrench))
		...
		playsound(src, 'sound/items/Ratchet.ogg', 80, 1, channel = "regular", time = 10)
