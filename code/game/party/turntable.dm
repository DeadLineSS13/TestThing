...
/obj/machinery/party/turntable
	...
	var/list/turntable_soundtracks = list(
		...
		new /datum/data/turntable_soundtrack ("DJ Eban",			"Амбиент разработки Ashen Sky",		'sound/turntable/shitzone.ogg',					            2610)
	)
	anchored = 1
	density = 1
...
//		dat += "<br><A href='?src\ref[src];turn_on=\ref[src]'>Turn On</A>"
	dat += "<br><A href='?src=\ref[src];skip=\ref[src]'>Skip</A>"
...
//	if(!istype(H.wear_id, /obj/item/device/pager))
//		say("Put on your pager.")
//		updateUsrDialog()
//		return

//	var/obj/item/device/pager/KPK = H.wear_id
...
//				return
	...
	if(href_list["order"])
		...
		if(!playing)
			say("Jukebox is turned off.","Jukebox is turned off.")
			return

//		if(alert("Do you want to play [TS.name]", "Turntable", "Yes", "No") == "No")
//			return
...
//			return
		...
		if(alert("Пропустить [track.name]?", "Turntable", "Yes", "No") == "No")
			return
...
/obj/machinery/party/turntable/proc/skip_song(var/datum/data/turntable_soundtrack/TS = pick(turntable_soundtracks))
	...
	track = TS
	say("Now playing: [track.f_name] - [track.name]","Now playing: [track.f_name] - [track.name]")
...
/obj/machinery/party/turntable/proc/turn_off()
	...
	for(var/client/C in melomans)
		//C.jukeboxplaying = 0
		if(C.mob)
			C.mob << sound(null, channel = SSchannels.get_reserved_channel(3), wait = 0)
		melomans.Remove(C)
...
/obj/machinery/party/turntable/proc/update_sound()
	...
	for (var/client/C in melomans)
		...
		if(!(C.mob))
			C << sound(null, channel = SSchannels.get_reserved_channel(3), wait = 0)
			melomans.Remove(C)
			continue
...
/obj/machinery/party/turntable/proc/create_sound(mob/M)
	if(!M.music || M.music.file != track.path)
		...
		S.repeat = 0
		S.channel = SSchannels.get_reserved_channel(3)
		S.falloff = 2
		S.wait = 0
		S.volume = 0
