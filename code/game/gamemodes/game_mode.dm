...
/datum/game_mode/proc/convert_roundtype()
	...
	for(var/mob/Player in GLOB.mob_list)
		if(Player.mind && Player.stat != DEAD && !isnewplayer(Player))// &&!isbrain(Player))
			living_crew += Player
	if(living_crew.len / GLOB.joined_player_list.len <= CONFIG_GET(number/midround_antag_life_check)) //If a lot of the player base died, we start fresh
		message_admins("Convert_roundtype failed due to too many dead people. Limit is [CONFIG_GET(number/midround_antag_life_check) * 100]% living crew")
		return null
...
//				return 1
	...
	if(world.time >= (CONFIG_GET(number/midround_antag_time_check) * 600))
		message_admins("Convert_roundtype failed due to round length. Limit is [CONFIG_GET(number/midround_antag_time_check)] minutes.")
		return null
...
//		return 1
	if(!round_converted && (!CONFIG_GET(keyed_list/continuous)[config_tag] || (CONFIG_GET(keyed_list/continuous)[config_tag] && CONFIG_GET(keyed_list/midround_antag)[config_tag]))) //Non-continuous or continous with replacement antags
		if(!continuous_sanity_checked) //make sure we have antags to be checking in the first place
			...
			if(!continuous_sanity_checked)
				...
				CONFIG_GET(keyed_list/continuous)[config_tag] = 1
				CONFIG_GET(keyed_list/midround_antag)[config_tag] = 0
		...
		if(!CONFIG_GET(keyed_list/continuous)[config_tag])
			return 1
