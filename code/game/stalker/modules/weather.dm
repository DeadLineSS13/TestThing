...
/datum/controller/subsystem/weather/Initialize()
	cooldown = 30000 + dice6(1) * 3000
	duration = 15000 + dice6(1) * 1500
	rain = new
//	rain = image('icons/stalker/structure/decor.dmi', icon_state = "rain", layer = 10)
	...
	rain.name = ""
	for(var/area/stalker/blowout/outdoor/A in world)
		for(var/turf/stalker/T in A.contents)
...
/datum/controller/subsystem/weather/proc/StartRain()
	...
	for(var/area/stalker/blowout/outdoor/A in world)
		for(var/turf/stalker/T in A.contents)
			T.rained = 1
			if(istype(T, /turf/stalker/floor/asphalt))
				var/turf/stalker/floor/asphalt/AT = T
				if(AT.AP)
					spawn(rand(200,400))
			...
			if(istype(T, /turf/stalker/floor/asphalt))
				...
				if(AT.AP)
					spawn(rand(3000,6000))
						AT.AP.icon_state = "[copytext("[AT.AP.icon_state]",1,5)]"
//			CHECK_TICK
/*
	for(var/client/C in GLOB.clients)
...
/mob/living/carbon/human/Login()
	...
	if(israin)
		start_rain_sound()
		for(var/turf/stalker/T in range(7,src))
			if(T.rained)
				var/image/I = image('icons/stalker/structure/decor.dmi', T, "rain", layer = 10)
				if(I)
...
/mob/living/carbon/human/proc/start_rain_sound()
	...
	rain.volume = 50
	rain.falloff = 2
	rain.channel = SSchannels.get_reserved_channel(12)
	rain.y = 1
	rain.wait = 1
	src << rain
	...
	rain_wind.falloff = 2
	rain_wind.channel = SSchannels.get_reserved_channel(13)
	rain_wind.y = 1
	...
	rain_thunder.channel = SSchannels.get_reserved_channel(14)
	rain_thunder.y = 1