...
/atom/movable/lighting_overlay/Initialize(mapload)
	...
	if (myturf.lighting_overlay)
		qdel(myturf.lighting_overlay, force = TRUE)
	myturf.lighting_overlay = src
	myturf.luminosity = 0

	needs_update = TRUE
	GLOB.lighting_update_overlays += src
...
/atom/movable/lighting_overlay/Destroy(var/force)
	if (force)
		...
		if (loc != myturf)
			...
			stack_trace("A lighting overlay was qdeleted with a different loc then it is suppose to have ([COORD(oldturf)] -> [COORD(newturf)])")
		if (isturf(myturf))
...
/atom/movable/lighting_overlay/proc/update()
	if (loc != myturf)
		if (loc)
			var/turf/oldturf = get_turf(myturf)
			var/turf/newturf = get_turf(loc)
			warning("A lighting overlay realised it's loc had changed in update() ([myturf]\[[myturf ? myturf.type : "null"]]([COORD(oldturf)]) -> [loc]\[[ loc ? loc.type : "null"]]([COORD(newturf)]))!")
