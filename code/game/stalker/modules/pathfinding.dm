...
/atom/movable/proc/new_walk_to(atom/target, move_delay = 1, min_dist = 0)
	var/static/walking = 0
	...
	spawn while(walking)
		...
		if(!base_StepTowards(target, min_dist))
			walking = 0
			break
		sleep(move_delay)
...
/mob/verb/move_here(mob/M in world)
	M.base_StepTowards(src.loc)
...
/////////////////////////////////////
atom/movable
	base_StepTowards(destination, minimum_distance = 0, pathSizeLimit = 100)
...
//			var/result = step_towards(owner, nextstep)
			...
			var/result = owner.Move(nextstep)
			if (!result)
...
//					world << "[type].base_StepTowards() can't reach turf [nextstep.x], [nextstep.y]; skipping to try next turf." // DEBUG
					nextstep = pathList[1]
					result = owner.Move(nextstep)
					if (!result)
						pathList.Cut(1,2)
			...
			if (get_dist(owner, destination_turf) <= minimum_distance)
				pathList = null
				return 1
			else
		...
		Path(distanceLimit = 0)
			...
			distanceEstimate = get_dist(starting_point, destination_turf)
...
//							theNode.costs = childCosts
						...
						if (isnull(childTurf) || isnull(destination_turf))
							...
							return
						estimatedDistance = get_dist(childTurf, destination_turf)
