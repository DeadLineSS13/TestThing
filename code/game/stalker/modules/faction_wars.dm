...
/obj/machinery/stalker/sidorpoint/proc/update_desc()
	if(controlled_by)
		...
		desc_ru = "Точка находитс&#255; под контролем группировки [get_faction(controlled_by)] на [control_percent]%"
		desc = "Point is under [controlled_by] control by [control_percent]%"

	else if(control_percent)

		desc_ru = "Точка находитс&#255; под контролем группировки [get_faction(capturing_faction)] на [control_percent]%"
		desc = "Point is under [capturing_faction] control by [control_percent]%"

	else

		desc_ru = "Точка свободна дл&#255; захвата"
