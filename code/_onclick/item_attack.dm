...
/atom/movable/attackby(obj/item/W, mob/living/user, params)
	...
	if(W && !(W.flags & NOBLUDGEON))
		visible_message("<span class='warning'>[user] has hit [src] with [W]!</span>")
...
/mob/living/attackby(obj/item/I, mob/user, params)
	...
	if(butcher_results && stat == DEAD) //can we butcher it?
		...
		if(sharpness)
			...
			playsound(loc, 'sound/weapons/slice.ogg', 50, 1, -1, channel = "regular", time = 10)
			if(do_mob(user, src, 80/sharpness))
				harvest(user)
			return
	I.attack(src, user, user.zone_selected)
...
/mob/living/proc/attacked_by(obj/item/I, mob/living/user, def_zone)
	...
	if(message_verb)
		visible_message("<span class='warning'>[attack_message]</span>",
		"<span class='warning'>[attack_message]</span>")
...
//		playsound(loc, 'sound/weapons/tap.ogg', get_clamped_volume(), 1, -1)//...play tap.ogg at get_clamped_volume()
	...
	if(user.lastattacked)
		user.lastattacked.remove_referenced(user)
	if(M.lastattacker)
		M.lastattacker.remove_referenced(M)

	user.lastattacked = M
	M.referenced_in(user, "lastattacked")
	M.lastattacker = user
	user.referenced_in(M, "lastattacker")
