...
/datum/controller/subsystem/ticker/Initialize(timeofday)
	...
	login_music = 'sound/stalker/largo.ogg'
...
//	for(var/i in start_landmarks_list)
...
/datum/controller/subsystem/ticker/proc/create_observers()
	for(var/mob/new_player/player in GLOB.player_list)
		if(player.ready == PLAYER_READY_TO_OBSERVE && player.mind)
			...
			addtimer(CALLBACK(player, /mob/new_player.proc/make_me_an_observer), 1)
*/
...
//	update_everything_flag_in_db()
	if(!round_end_sound)
		round_end_sound = 'sound/misc/valim.wav'
	world << sound(round_end_sound, channel = SSchannels.get_reserved_channel(1))
