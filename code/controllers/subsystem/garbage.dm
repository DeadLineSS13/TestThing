SUBSYSTEM_DEF(garbage)
	...
	var/list/collection_timeout = list(30 SECONDS, 0, 0)	// deciseconds to wait before moving something up in the queue to the next level
...
/datum/controller/subsystem/garbage/proc/Everything_to_HD()
	var/list/queue = queues[GC_QUEUE_CHECK]
	for(var/refID in queue)
		if(!refID)
			continue
		...
		if (!D || D.gc_destroyed != GCd_at_time) // So if something else coincidently gets the same ref, it's not deleted by mistake
			++gcedlasttick
			++totalgcs
			pass_counts[GC_QUEUE_CHECK]++
			#ifdef TESTING
			reference_find_on_fail -= refID		//It's deleted we don't care anymore.
			#endif
			continue

		Queue(D, GC_QUEUE_HARDDELETE)
		CHECK_TICK

	queue = queues[GC_QUEUE_HARDDELETE]
	for(var/refID in queue)
		if(!refID)
			continue
...
/client/verb/find_refs(var/ref as text)
	set category = "Debug"
	...
	if(!D)
		src << "There's no such obj in the world!"
		return
...
/datum/proc/find_references(skip_alert)
	...
	for(var/client/thing in world) //clients
		DoSearchVar(thing, "World -> [thing]")
	...
	if(istype(X, /datum))
		...
		for(var/varname in L)
			...
			if(variable == src)
				...
				DoSearchVar(variable, "[varname] -> list", recursive_limit-1)
...
/*
/client/verb/check_ref(var/ref as text)
	set category = "Debug"
