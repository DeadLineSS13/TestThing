/mob/living/simple_animal
	...
	var/list/speak_emote_ru = list()
	var/speak_chance = 0
	var/list/emote_hear = list()	//Hearable emotes
	var/list/emote_see = list()		//Unlike speak_emote, the list of things in this variable only show by themselves with no spoken text. IE: Ian barks, Ian yaps
	var/list/emote_see_ru = list("срет")
				...
				if((emote_hear && emote_hear.len) || (emote_see && emote_see.len))
					...
					if(randomValue <= speak.len)
						...
						if(emote_see && randomValue <= emote_see.len)
							emote("me", 1, pick(emote_see), pick(emote_see_ru))
						else
...
/mob/living/simple_animal/proc/handle_automated_sounds()
	if(idle_sounds)
		if(rand(0,200) < speak_chance)
			...
			playsound(src, s, 65, 1, channel = "regular", time = 20)
...
/mob/living/simple_animal/emote(act, m_type=1, message = null, message_ru = null)
	if(stat)
		return
	if(act == "scream")
		message = "makes a loud and pained whimper" //ugly hack to stop animals screaming when crushed :P
		act = "me"
	..(act, m_type, message, message_ru)
...
/mob/living/simple_animal/bullet_act(obj/item/projectile/Proj)
	if(!Proj)
		return
	apply_damage(Proj.damage_add, Proj.damage_type)
	Proj.on_hit(src)
	return 0
*/
...
/mob/living/simple_animal/attack_hand(mob/living/carbon/human/M)
	...
	switch(M.a_intent)
		...
		if("harm", "disarm")
			...
			playsound(loc, "punch", 25, 1, -1, channel = "regular", time = 10)
	...
	if(loot.len)
		if(random_loot)
			...
			for(var/i in loot)
				new i(loc)
	*/
	playsound(src, death_sound, 75, 1, channel = "regular", time = 20)
	if(deathmessage && !gibbed)
		visible_message("<span class='danger'>[deathmessage]</span>", message_ru = "<span class='danger'>[deathmessage_ru]</span>")
