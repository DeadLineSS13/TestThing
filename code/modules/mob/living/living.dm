...
/mob/living/proc/MobBump(mob/M)
	...
	if(istype(M, /mob/living))
		for(var/mob/MM in range(1,M))
			if( ((MM.pulling == M && ( M.restrained() && !( MM.restrained() ) && MM.stat == CONSCIOUS)) || pulledby) )
				if ( !(world.time % 5) )
...
*/
//same as above
/mob/living/pointed(atom/A as mob|obj|turf in view(14, src))
...
/mob/living/proc/adjustPsyLoss(amount)
	if(status_flags & GODMODE)	return 0
	psyloss = min(max(psyloss + amount, 0), int*2)
	handle_regular_status_updates()
...
/mob/living/proc/getEnduranceLoss()
	return enduranceloss

/mob/living/proc/adjustEnduranceLoss(amount)
	if(status_flags & GODMODE)
		return 0
	enduranceloss = min(max(enduranceloss + amount, 0), hlt*hlt*10)

/mob/living/proc/setEnduranceLoss(amount)
	if(status_flags & GODMODE)
		return 0
	enduranceloss = amount
...
/mob/living/proc/mob_sleep()
	...
	if(sleeping)
		src << "You're waking up."
		SetSleeping(0, 1)
		return
	else
		if(alert(src, "You sure you want to sleep for a while?", "Sleep", "Yes", "No") == "Yes")
			var/sleep_amount = (input(usr, "How long do you want to sleep in minutes?", "Sleeping") as  null|num)*30
			SetSleeping(sleep_amount) //Short nap
	update_canmove()
...
/mob/living/proc/lay_down()
	...
	if(weakened || stunned || paralysis || sleeping || magniting || pulledby)
		return
	...
	if(resting)
		...
		var/time = 10
		if(stamina_coef)
			time *= 1*2**(stamina_coef)
		if(flags & IN_PROGRESS)
			return
		flags += IN_PROGRESS
		if(!do_mob(src, src, time, 1))
			flags &= ~IN_PROGRESS
			return
		flags &= ~IN_PROGRESS
		if(resting)
...
//		D.cure(0)
	...
	if(ishuman(src))
		...
		for(var/obj/item/organ/limb/O in human_mob.organs)
			O.bleeding = 0
		human_mob.restore_blood()
		human_mob.remove_all_embedded_objects()
		human_mob.update_damage_hud()
	...
	if (!cuff_dragged && pulling && !throwing && (get_dist(src, pulling) <= 1 || pulling.loc == loc))
		...
		if (pulling && pulling.loc)
			if(!isturf(pulling.loc))
				...
				if(GLOB.Debug)
					world.log <<"pulling disappeared? at [__LINE__] in mob.dm - pulling = [pulling]"
...
*/
	...
	if (s_active && !(s_active in contents) && !(s_active.loc in contents) && loc != oldloc)
		// It's ugly. But everything related to inventory/storage is. -- c0
		s_active.close(src)

//	if(oldloc != newloc)
	for(var/client/C in in_vision_cones)
		if(src in C.hidingAtoms)
			var/turf/T = get_turf(src)
			var/image/I = image('icons/effects/footstepsound.dmi', loc = T, icon_state = "default", layer = 20)
			C.images += I
			spawn(4)
				if(C)
					C.images -= I
...
/mob/living/movement_delay()
	...
	switch(m_intent)
		if("run")
			if(drowsyness > 0)
				. += 6
			. += CONFIG_GET(number/movedelay/run_delay)
		if("walk")
...
/mob/living/verb/resist()
	...
	if(!stat && canmove && !restrained() && pulledby)
		for(var/obj/O in requests)
			qdel(O)
		visible_message("<span class='warning'>[src] tried to resist!</span>")
		var/def_dice
		if(str > agi)
			def_dice = str - dice6(3)
		else
			def_dice = agi - dice6(3)
		var/atack_dice = pulledby.str + lying ? 2 : 0 - dice6(3)
		if(!rolld(def_dice, atack_dice))
//			qdel(G)
			visible_message("<span class='warning'>[src] successfully resists!</span>")
//TODO:			pulledby.
		return

	if(istype(loc, /obj/structure/stalker/car/army_moveable))
		var/obj/structure/stalker/car/army_moveable/AM = loc
		AM.attack_hand(src)
		return
	...
	if(ishuman(src))
		...
		if(H.sticky_pit)
			if(H.shoes)
				for(var/obj/effect/asphalt_pit/AP in H.loc)
					var/icon/I = new('icons/mob/feet.dmi', H.shoes.icon_state, H.dir)
					I.Shift(NORTH, 12)
					AP.add_overlay(image(I, AP))
					qdel(I)
				qdel(H.shoes)
				H.sticky_pit = 0
				H.pixel_x = 0
...
/mob/living/stripPanelUnequip(obj/item/what, mob/who, where)
	...
	who.direct_visible_message("<span class='danger'>DOER tries to remove TARGET's [what.name].</span>",
								...
								"<span class='danger'>DOER �������� ����� � TARGET [what.name_ru].</span>",
								"<span class='danger'>DOER �������� ����� � TARGET [what.name_ru].</span>","danger",src,who)
	what.add_fingerprint(src)
	if(do_mob(src, who, what.strip_delay))
		if(what && what == who.get_item_by_slot(where) && Adjacent(who))
			who.unEquip(what, 0, 0)
			put_in_hands(what)
...
/mob/living/stripPanelEquip(obj/item/what, mob/who, where)
	...
	if(what && (what.flags & NODROP))
		...
		return
	if(what)
		if(!what.mob_can_equip(who, where, 1, 0))
		...
		direct_visible_message("<span class='notice'>DOER tries to put [what.name] on TARGET.</span>",
					message_ru = "<span class='notice'>DOER �������� ����� [what.name_ru] �� TARGET.</span>",
					span_class = "notice", doer = src, target = who)
		if(do_mob(src, who, what.put_on_delay))
			if(what && Adjacent(who))
				unEquip(what, 1, 0)
				who.equip_to_slot_if_possible(what, where, 0, 1, delay = 0)
