...
/mob/living/Hear(message, atom/movable/speaker, message_langs, raw_message, radio_freq, list/spans, msg_ru)
	...
	if(ishuman(speaker))
		...
		if(H.tangled)
			...
			var/msg_to_find
			var/lenght_change = 0
			var/list/words_on_replace = list("неа", "дальше", "нет","откажусь", "отказываюсь","возражаю",   "против",  " прав",   "верх","влев", "налев", "север","запад", "вперёд","враг","не ","не")
			var/list/words_to_replace = list("окей","обратно","да", "соглашусь","соглашаюсь", "не возражаю","согласен"," не прав","низ", "вправ","направ","юг",   "восток","назад", "друг","",   ""  )
...
/mob/living/say_quote(input, list/spans)
	...
	if (stuttering)
		if(client)
			return client.select_lang("заикается, \"[tempinput]\"", "stammers, \"[tempinput]\"")
	return ..()