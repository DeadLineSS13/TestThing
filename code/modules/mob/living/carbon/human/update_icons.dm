...
/mob/living/carbon/human/update_inv_head_hard()
	remove_overlay(HEAD_HARD_LAYER)

//	if(!get_bodypart("head")) //Decapitated
//		return

	if(client && hud_used && hud_used.inv_slots[slot_head_hard])
		var/obj/screen/inventory/inv = hud_used.inv_slots[slot_head_hard]
		inv.update_icon()

	if(head_hard)
		var/image/standing = head_hard.build_worn_icon(state = head_hard.icon_state, default_layer = HEAD_HARD_LAYER, default_icon_file = 'icons/mob/head.dmi')
		overlays_standing[HEAD_HARD_LAYER] = standing
		head_hard.screen_loc = ui_head_hard
		if(client && hud_used && hud_used.hud_shown)
			if(hud_used.inventory_shown)
				client.screen += head_hard
		update_observer_view(head_hard,1)


	apply_overlay(HEAD_HARD_LAYER)
...
/mob/living/carbon/human/update_inv_wear_suit()
	...
	if(istype(wear_suit, /obj/item/clothing/suit))
		...
		var/image/standing2 = wear_suit.build_worn_icon(state = "[wear_suit.icon_state]_b", default_layer = SUIT_BEHIND_LAYER, default_icon_file = 'icons/mob/suit.dmi')
		overlays_standing[SUIT_LAYER]	= standing
		overlays_standing[SUIT_BEHIND_LAYER]	= standing2
...
//			drop_all_held_items()
	...
	apply_overlay(SUIT_BEHIND_LAYER)

/mob/living/carbon/human/update_inv_wear_suit_hard()
	remove_overlay(SUIT_HARD_LAYER)

	if(client && hud_used)
		var/obj/screen/inventory/inv = hud_used.inv_slots[slot_wear_suit_hard]
		inv.update_icon()

	if(istype(wear_suit_hard, /obj/item/clothing))
		wear_suit_hard.screen_loc = ui_oclothing_hard
		if(client && hud_used && hud_used.hud_shown)
			if(hud_used.inventory_shown)
				client.screen += wear_suit_hard
		update_observer_view(wear_suit_hard,1)

		var/image/standing = wear_suit_hard.build_worn_icon(state = wear_suit_hard.icon_state, default_layer = SUIT_HARD_LAYER, default_icon_file = 'icons/mob/suit.dmi')
		overlays_standing[SUIT_HARD_LAYER]	= standing
...
/mob/living/carbon/human/update_hud_back2(obj/item/I)
	I.screen_loc = ui_back2
	if(client && hud_used && hud_used.hud_shown)
		client.screen += I
	update_observer_view(I)
