...
/datum/species/proc/handle_hair(mob/living/carbon/human/H, forced_colour)
	...
	if(H.facial_hair_style && (FACEHAIR in specflags) && !facialhair_hidden)
		...
		HC = GLOB.hair_colors_list[H.facial_hair_color]
		if(S)
			var/image/img_facial_s = image("icon" = S.icon, "icon_state" = "[S.icon_state]_s", "layer" = -HAIR_LAYER)

			if(!forced_colour)
				if(HC)
					img_facial_s.color = HC.color
				else
					img_facial_s.color = "#000000"
			else
	...
	if(!hair_hidden)
		if(H.hair_style && (HAIR in specflags))
			...
			HC = GLOB.hair_colors_list[H.hair_color]
			if(S)
				...
				if(!forced_colour)
					if(HC)
						img_hair_s.color = HC.color
					else
						img_hair_s.color = "#000000"
				else
...
/datum/species/proc/can_equip(obj/item/I, slot, disable_warning, mob/living/carbon/human/H)
	...
	switch(slot)
		...
		if(slot_wear_suit_hard)
			if(H.wear_suit_hard)
				return 0
			if(!(I.slot_flags & SLOT_OCLOTHING_HARD))
				return 0
			return 1
		if(slot_head_hard)
			if(H.head_hard)
				return 0
			if(!(I.slot_flags & SLOT_HEAD_HARD))
				return 0
			return 1
		if(slot_gloves)
			if(H.gloves)
...
/datum/species/proc/handle_chemicals_in_body(mob/living/carbon/human/H)
	if(istype(get_area(H), /area/stalker/kyrilka))
		return
	...
	if (H.nutrition > 0 && H.stat != DEAD)
		...
		if(H.satiety < 0)
			...
			if(prob(round(-H.satiety/40)))
				H.Jitter(5)
			hunger_rate = 3 * HUNGER_FACTOR * (1 + H.artefacts_effects["starvation"]/100)
...
//			H << H.client.select_lang("<span class='notice'>¬ы чувствуете себ€ бодрым.</span>","<span class='notice'>You feel vigorous.</span>")
			H.metabolism_efficiency = 1.25
	else if(H.nutrition < NUTRITION_LEVEL_STARVING + 50)
//		if(H.metabolism_efficiency != 0.8)
//			H << H.client.select_lang("<span class='notice'>¬ы чувствуете себ€ в€ло.</span>","<span class='notice'>You feel sluggish.</span>")
		H.metabolism_efficiency = 0.8
	else
//		if(H.metabolism_efficiency == 1.25)
//			H << H.client.select_lang("<span class='notice'>¬ы больше не чувствуете бодрости.</span>","<span class='notice'>You no longer feel vigorous.</span>")
		H.metabolism_efficiency = 1
...
//				H.clear_alert("blind")
		...
		if( H.disabilities & NEARSIGHT && !istype(H.glasses, /obj/item/clothing/glasses/regular) )
			H.overlay_fullscreen("impared", /obj/screen/fullscreen/impaired)
		if(H.eye_blurry)
	...
	if(H.healthdoll)
		...
		if(H.stat == DEAD)
			...
			for(var/obj/item/organ/limb/L in H.organs)
				var/damage = L.burn_dam + L.crush_dam + L.cut_dam
...
*/
	switch(H.nutrition)
		if(0 to NUTRITION_LEVEL_STARVING)
			H.hud_used.hunger.icon_state = "food_2"
		if(NUTRITION_LEVEL_STARVING to NUTRITION_LEVEL_HUNGRY)
			H.hud_used.hunger.icon_state = "food_1"
		else
			H.hud_used.hunger.icon_state = "food"
...
/datum/species/proc/handle_mutations_and_radiation(mob/living/carbon/human/H)
	...
	if(!(RADIMMUNE in specflags))
		//if(H.radiation)
			//if (H.radiation > 100)
		//		H << "<span class='danger'>ќпасный уровень радиации. Ќеобходима защита, чтобы пройти.</span>"
		//		H.Weaken(10)
		//		H.emote("collapse")
...
//			var/obj/item/organ/limb/affecting = H.get_organ(ran_zone(M.zone_selected))
			var/diceM = (M.agi + M.skills["melee"]) - dice6(3)
			var/diceH
			if(H.str > H.agi)
				diceH = (H.str + H.skills["melee"]) - dice6(3)
			else
				diceH = (H.agi + H.skills["melee"]) - dice6(3)
			if((diceM-diceH) > 0 && (diceM-diceH) < 4)
...
*/
				if(!talked)	//BubbleWrap
					if(H.drop_item())
						H.direct_visible_message("<span class='danger'>DOER has disarmed TARGET!</span>",\
												"<span class='danger'>DOER has disarmed you!</span>",\
												"<span class='danger'>DOER обезоружил TARGET!</span>", \
												"<span class='danger'>DOER обезоружил теб€!</span>",\
												"danger", M, H)
				playsound(H, 'sound/weapons/thudswoosh.ogg', 50, 1, -1, channel = "regular", time = 10)
				return

			else if((diceM-diceH) >= 4)
				playsound(H, 'sound/weapons/thudswoosh.ogg', 50, 1, -1, channel = "regular", time = 10)
				H.direct_visible_message("<span class='danger'>DOER has pushed TARGET!</span>",
								"<span class='danger'>DOER has pushed you!</span>",
								"<span class='danger'>DOER повалил на землю TARGET!</span>",
								"<span class='danger'>DOER повалил теб€ на землю!</span>",
								"danger", M, H)
//				H.apply_effect(2, WEAKEN, H.run_armor_check(affecting, "melee", "Your armor prevents your fall!", "Your armor softens your fall!"))
			...
			playsound(H, 'sound/weapons/punchmiss.ogg', 25, 1, -1, channel = "regular", time = 10)
			H.direct_visible_message("<span class='danger'>DOER attempted to disarm TARGET!</span>",\
							"<span class='danger'>DOER attemped to disarm you!</span>",\
							"<span class='danger'>DOER попыталс€ обезоружить TARGET!</span>",\
							"<span class='danger'>DOER попыталс€ теб€ обезоружить!</span>",
							"danger", M, H)
	return
...
/datum/species/proc/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked, mob/living/carbon/human/H, atom/damager)
	if(istype(get_area(H), /area/stalker/kyrilka))
		return
	...
	switch(damagetype)
		if(BRUTE)
			...
			if(organ.take_damage(damage*brutemod, 0, 0))
...
//			shake_camera(H, damage, Clamp(damage/100, 0.1, 1))
		...
		if(CUT)
			H.damageoverlaytemp = 20
			if(organ.take_damage(0, damage*brutemod, 0))
				H.update_damage_overlays(0)
		if(BURN)
			H.damageoverlaytemp = 20
			if(organ.take_damage(0, 0, damage*burnmod))
				H.update_damage_overlays(0)
//			shake_camera(H, damage, Clamp(damage/100, 0.1, 1))
		if(TOX)
...
//			shake_camera(H, damage, Clamp(damage/100, 0.1, 1))
	...
	if(H.health > H.maxHealth-H.str)
	else if(H.health > H.maxHealth-H.str*2 && H.health <= H.maxHealth-H.str)
		if(damage >= round(H.str/2))
			if(!H.boosted)
				consious_dice(1, H)
	else if(H.health > H.maxHealth-H.str*3 && H.health <= H.maxHealth-H.str*2)
		if(damage >= round(H.str/2))
			if(!H.boosted)
				consious_dice(2, H)
		if(H.death_dices < 2)
			death_dice(2, H)
			H.death_dices = 2
	else if(H.health > H.maxHealth-H.str*4 && H.health <= H.maxHealth-H.str*3)
		if(damage >= round(H.str/2))
			if(!H.boosted)
				consious_dice(3, H)
		if(H.death_dices < 3)
			death_dice(3, H)
			H.death_dices = 3
	else if(H.health > H.maxHealth-H.str*5 && H.health <= H.maxHealth-H.str*4)
		if(damage >= round(H.str/2))
			if(!H.boosted)
				consious_dice(4, H)
		if(H.death_dices < 4)
			death_dice(4, H)
			H.death_dices = 4
	else if(H.health > H.maxHealth-H.str*6 && H.health <= H.maxHealth-H.str*5)
		if(damage >= round(H.str/2))
			if(!H.boosted)
				consious_dice(5, H)
		if(H.death_dices < 5)
...
/datum/species/proc/consious_dice(var/penalty, mob/living/carbon/human/H)
	var/dice = dice6(3)
//	world << "Consious dice: [dice]"
//	world << "Consious parm: [H.hlt+1-penalty]"
	if(H.rolld(dice, H.hlt+1-penalty))
		var/timing = rand(100, 300)
		H.SetParalysis(timing)
		spawn(timing)
			if(H)
				H.SetParalysis(0)

/datum/species/proc/death_dice(var/penalty, mob/living/carbon/human/H)
	var/dice = dice6(3)
//	world << "Dead dice: [dice]"
//	world << "Dead parm: [H.hlt+2-penalty]"
	if(H.rolld(dice, H.hlt+2-penalty))
		H.handle_hud_icons_health(0)
		if(penalty == 20)
			H.death(0, 0)
		else
			H.death(0, 1)
