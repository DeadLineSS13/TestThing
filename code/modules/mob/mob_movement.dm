...
//		next_move_dir_sub = 0
	if(!mob || !mob.loc)
		return 0
	if(mob.zoomed && mob.loc != n)
		mob.toggle_zoom()
	if(mob && eye != mob && mob.loc != n)
		mob.set_focus(mob)
		mob.clear_fullscreen("hatch", 0)
		mob.screens["fov"].alpha = 255
		mob.reset_view(null)
		return 0
	if(mob.notransform)
		...
		for(var/mob/M in orange(1, mob))
			if(M.pulling == mob)
				if(!M.incapacitated() && mob.Adjacent(M))
					src << "<span class='warning'>You can't.</span>"
	...
	if(locate(/obj/item/weapon/grab, locate(/obj/item/weapon/grab, mob.grabbed_by.len)))
		...
		for(var/obj/item/weapon/grab/G in mob.grabbed_by)
			...
			if(G.state == GRAB_AGGRESSIVE)
				...
				if(!prob(25))
					return 1
				mob.visible_message(message = "<span class='warning'>[mob] has broken free of [G.assailant]'s grip!</span>", message_ru = "<span class='warning'>[mob] вырвался из хватки [G.assailant]!</span>")
				qdel(G)
			...
			if(G.state == GRAB_NECK)
				...
				if(!prob(5))
					return 1
				mob.visible_message(message = "<span class='warning'>[mob] has broken free of [G.assailant]'s headlock!</span>", message_ru = "<span class='warning'>[mob] вырвался из хватки [G.assailant]!</span>")
				qdel(G)
*/
	return 0
...
/mob/proc/Move_Pulled(atom/A)
	if(!pulling || !canmove || restrained())
		return
	if(get_dist(pulling, A) > 1)
		return
	if(!pulling.Adjacent(src))
		pulling.pulledby = null
		pulling = null
		return
	pulling.Move(A, get_dir(pulling.loc, A))
	return
