/datum/admins/proc/CheckAdminHref(href, href_list)
	return TRUE
//Просто потому что идите нахуй, мне это нужно
/*
...
*/
		...
		if(!CONFIG_GET(keyed_list/continuous)[SSticker.mode.config_tag])
			CONFIG_GET(keyed_list/continuous)[SSticker.mode.config_tag] = 1
		else
			CONFIG_GET(keyed_list/continuous)[SSticker.mode.config_tag] = 0

		message_admins("<span class='adminnotice'>[key_name_admin(usr)] toggled the round to [CONFIG_GET(keyed_list/continuous)[SSticker.mode.config_tag] ? "continue if all antagonists die" : "end with the antagonists"].</span>")
		check_antagonists()
/*
...
*/
		...
		var/timer = input("Enter new maximum time",, CONFIG_GET(number/midround_antag_time_check)) as num
		if(timer)
...
//				feedback_inc("ban_appearance",1)
				...
				if(CONFIG_GET(string/banappeals))
					M << "<span class='danger'>To try to resolve this matter head to [CONFIG_GET(string/banappeals)]</span>"
				else
...
//						feedback_inc("ban_job_unban",1)
	...
	//Watchlist
		...
		world << "<span class='adminnotice'><b>The mode is now: [GLOB.master_mode]</b></span>"
...
//		output_ai_laws()
		...
		if(isliving(M))
			...
			health_description += "<BR>Oxy: [L.getOxyLoss()] - Tox: [L.getToxLoss()] - Fire: [L.getFireLoss()] - Brute: [L.getBruteLoss()] - Stamina: [L.getStaminaLoss()]"
		else
