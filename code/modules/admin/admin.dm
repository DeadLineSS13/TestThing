...
//		feedback_add_details("admin_verb","SN") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
		return 1
	else if (SSticker.current_state == GAME_STATE_STARTUP)
...
/client/proc/adminGreet(logout)
	if(SSticker && SSticker.current_state == GAME_STATE_PLAYING)
		var/string
		if(logout && config && CONFIG_GET(flag/announce_admin_logout))
			string = pick(
				"Admin logout: [key_name(src)]")
		else if(!logout && config && CONFIG_GET(flag/announce_admin_login) && (prefs.toggles & ANNOUNCE_LOGIN))
			string = pick(
