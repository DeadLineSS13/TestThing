...
/datum/preferences
	...
	var/facial_hair_color = ""		//Facial hair color
...
/datum/preferences/New(client/C)
	...
	if(!loaded_preferences_successfully)
		save_preferences()
	save_character(0)		//let's save this new random character so it doesn't keep generating new ones.
	return
...
//			dat += "<a href='?_src_=prefs;preference=all'>Always Random Body: [be_random_body ? "Yes" : "No"]</A><br>"
			...
			dat += "<table width='100%'><tr><td width='24%' valign='top'><br>"
...
/*
		if (1) // Game Preferences
			...
			// dat += "<b>tgui Style:</b> <a href='?_src_=prefs;preference=tgui_fancy'>[(tgui_fancy) ? "Fancy" : "Fancy"]</a><br>"
			...
			dat += "<b>Ghost ears:</b> <a href='?_src_=prefs;preference=ghost_ears'>[(chat_toggles & CHAT_GHOSTEARS) ? "Nearest Creatures" : "Nearest Creatures"]</a><br>"
			dat += "<b>Ghost sight:</b> <a href='?_src_=prefs;preference=ghost_sight'>[(chat_toggles & CHAT_GHOSTSIGHT) ? "Nearest Creatures" : "Nearest Creatures"]</a><br>"
			dat += "<b>Ghost whispers:</b> <a href='?_src_=prefs;preference=ghost_whispers'>[(chat_toggles & CHAT_GHOSTWHISPER) ? "Nearest Creatures" : "Nearest Creatures"]</a><br>"
...
*/
		if(2)	//Skills
			...
			switch(user.client.language)
				if("English")
					dat += "<h2>Traits</h2>"
					dat += "<b>Education: </b><a href ='?_src_=prefs;preference=education;task=input'>[education]</a> <a href ='?_src_=prefs;preference=e_info;task=output'>\[?\]</a><br>"
					dat += "<b>Job: </b><a href ='?_src_=prefs;preference=profession;task=input'>[profession]</a> <a href ='?_src_=prefs;preference=p_info;task=output'>\[?\]</a><br>"
					dat += "<b>Lifestyle: </b><a href ='?_src_=prefs;preference=lifestyle;task=input'>[lifestyle]</a> <a href ='?_src_=prefs;preference=l_info;task=output'>\[?\]</a><br>"
					dat += "<b>Free Trait: </b><a href ='?_src_=prefs;preference=trait;task=input'>[trait]</a> <a href ='?_src_=prefs;preference=t_info;task=output'>\[?\]</a>"
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("hair")
					if(hair_style == "Lysiy")
						return
					var/new_hair = input(user, "Choose your character's hair colour:", "Character Preference") as null|anything in (user.client.get_donation("Gay Hair") ? GLOB.hair_colors_list_normal + GLOB.hair_colors_list_donate : GLOB.hair_colors_list_normal)
					hair_color = new_hair
				...
				if("facial")
					if(facial_hair_style == "Britiy")
						return
					var/new_facial = input(user, "Choose your character's facial-hair colour:", "Character Preference") as null|anything in (user.client.get_donation("Gay Hair") ? GLOB.hair_colors_list_normal + GLOB.hair_colors_list_donate : GLOB.hair_colors_list_normal)
				...
				if("deity_name")
					...
					if(new_deity_name)
						...
						user << "<font color='red'>Invalid name. Your name should be at least 2 and at most [MAX_NAME_LEN] characters long. It may only contain the characters A-Z, a-z, -, ' and .</font>"
				if("education")
					var/new_education = input(user, "Your education is...", "Traits") as null|anything in GLOB.education_list
					if(new_education)
						education = new_education
				if("profession")
					var/new_profession = input(user, "Your profession is...", "Traits") as null|anything in GLOB.profession_list
					if(new_profession)
						profession = new_profession
				if("lifestyle")
					var/new_lifestyle= input(user, "Your lifestyle is...", "Traits") as null|anything in GLOB.lifestyle_list
					if(new_lifestyle)
						lifestyle = new_lifestyle
				if("trait")
					var/new_trait = input(user, "Your trait is...", "Traits") as null|anything in GLOB.trait_list
					if(new_trait)
						trait = new_trait
				if("beer")
					var/new_beer = input(user, "Your favourite beer is...", "Traits") as null|anything in GLOB.beer
					if(new_beer)
						custom_beer = new_beer
			...
			switch(href_list["preference"])
				...
				if("lobby_music")
					...
					if(toggles & SOUND_LOBBY)
						user << sound(SSticker.login_music, repeat = 0, wait = 0, volume = 85, channel = SSchannels.get_reserved_channel(1))
					else
						user.stopLobbySound()
								...
								switch(alert("Ты действительно хочешь сохранить персонажа? Внести изменениЯ будет нельзЯ!","Сохранение персонажа","Да","Нет"))
									if("Да")
										...
										if(user.client.loadout)
											user.client.loadout.build_loadout()
							else
								switch(alert("Do you really want to save your character? You won't be able to change it!","Save character","Yes","No"))
									if("Yes")
										save_character()
										save_preferences()
										if(user.client.loadout)
						...
						if("Russian")
							switch(alert("Ты действительно хочешь сбросить персонажа? Отменить изменение будет нельзЯ!","Сохранение персонажа","Да","Нет"))
								if("Да")
									...
									saved = 0
									loaded[default_slot] = 0
									save_character(0)
									save_preferences()
									if(user.client.loadout)
										del user.client.loadout
										fdel("data/player_saves/[copytext(user.client.ckey,1,2)]/[user.client.ckey]/loadout.sav")
										user.client.loadout = new /datum/loadout(user.client)
						else
							switch(alert("Do you really want to reset your character? You won't be able to cancel it!","Reset character","Yes","No"))
								if("Yes")
									random_character()
									real_name = random_unique_name(gender)
									saved = 0
...
/datum/preferences/proc/copy_to(mob/living/carbon/human/character, icon_updates = 1)
	...
	if(hair_style == "Lysiy")
		character.hair_color = "Bald"
	else
		character.hair_color = hair_color
	if(facial_hair_style == "Britiy")
		character.facial_hair_color = "Bald"
	else
...
//	chosen_species = /datum/species/human
	character.set_species(/datum/species/human, icon_update=0)
	...
	character.update_traits()

	if(custom_beer != "Random")
		character.favourite_beer = custom_beer
