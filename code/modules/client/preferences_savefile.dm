	...
	if(current_version < 9)		//an example, underwear were an index for a hardcoded list, converting to a string
		if(gender == MALE)
			...
			switch(underwear)
				...
				if(13)	underwear = "Nude"
		if(!(pref_species in GLOB.species_list))
...
//	else
	...
	S["profession_"]		>> profession
	S["lifestyle"]			>> lifestyle
	S["trait"]				>> trait
	S["custom_beer"]		>> custom_beer
...
//	facial_hair_color			= sanitize_hexcolor(facial_hair_color, 3, 0)
	...
	education = sanitize_inlist(education, GLOB.education_list)
	profession = sanitize_inlist(profession, GLOB.profession_list)
	lifestyle = sanitize_inlist(lifestyle, GLOB.lifestyle_list)
	trait = sanitize_inlist(trait, GLOB.trait_list)
...
/datum/preferences/proc/save_character(save = 1)
	if(!path)				return 0
	var/savefile/S = new /savefile(path)
	if(!S)					return 0
