...
/client/Topic(href, href_list, hsrc)
	...
	//Logs all hrefs
	if(config && CONFIG_GET(flag/log_admin) && href_logfile)
	...
	switch(href_list["_src_"])
		...
		if("loadout")	return loadout.process_link(usr,href_list)
...
*/
	...
	if (isnum(player_age) && player_age == -1) //first connection
		...
		if (CONFIG_GET(number/notify_new_player_age) >= 0)
...
//				send2irc_adminless_only("New-user", "[key_name(src)] is connecting for the first time!")
	...
	else if (isnum(player_age) && player_age < CONFIG_GET(number/notify_new_player_age))
...
/*	if(prefs.lastchangelog != changelog_hash) //bolds the changelog button on the interface so we know there are updates.
	...
	if (config && CONFIG_GET(flag/autoconvert_notes))
	...
	if(get_donation("Character Slot"))
	...
	verb //Verbs can be accessed by the macros (using key presses), so we need to define verbs (not procs) for our key presses.
		ToggleFullscreen()
			...
			if(fullscreen) //If fullscreen == 1 (TRUE)
				...
				winset(src, "mainwindow", "is-maximized=false;can-resize=true;titlebar=true;menus=menu") //Set window to normal size.

client/proc/toggle_popups()
	set name = "Toggle RMB Popups"
