...
/obj/item/weapon/clipboard/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/weapon/paper))
		...
		toppaper.attackby(usr.get_active_held_item(), usr)
		update_icon()
...
/obj/item/weapon/clipboard/Topic(href, href_list)
	...
	if(usr.contents.Find(src))
		...
		if(href_list["addpen"])
			if(!haspen)
				if(istype(usr.get_active_held_item(), /obj/item/weapon/pen))
					var/obj/item/weapon/pen/W = usr.get_active_held_item()
					if(!usr.unEquip(W))
						return
					W.loc = src
					haspen = W
