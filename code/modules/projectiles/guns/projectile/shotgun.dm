...
/obj/item/weapon/gun/projectile/shotgun/attackby(obj/item/A, mob/user, params)
	...
	if(!user.is_holding(src))
		return
	var/num_loaded = magazine.attackby(A, user, params, 1, 0, 1)
	if(num_loaded)
		if(jam)
			jam = 0
...
/obj/item/weapon/gun/projectile/shotgun/boltaction/examine(mob/user)
	..()
	user << user.client.select_lang("Ты передергиваешь затвор, [bolt_open ? "открывая" : "закрывая"] его.", "The bolt is [bolt_open ? "open" : "closed"].")
