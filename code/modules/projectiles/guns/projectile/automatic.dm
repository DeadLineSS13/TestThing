	...
	if(istype(A, /obj/item/ammo_box/magazine))
		...
		if(istype(AM, mag_type))
			if(magazine)
				...
				if(prob(5) && !jam)
					jam = 1
					user << "<span class='warning'>Оружие заклинило. Его необходимо перезарядить. Нормально перезарядить.</span>"
			else
...
/obj/item/weapon/gun/projectile/automatic/proc/burst_select()
	...
	switch(select)
		if(0)
			...
			burst_size = 1
			fire_delay = 0
			user << user.client.select_lang("<span class='notice'>Теперь оружие стреляет одиночными.</span>","<span class='notice'>You are now using single-shot mode.</span>")

		if(1)
			select = 0
			burst_size = initial(burst_size)
			fire_delay = initial(fire_delay)
			user << user.client.select_lang("<span class='notice'>Теперь оружие стреляет очередями.</span>","<span class='notice'>You are now using burst mode.</span>")

		if(2)
			select = 0
			burst_size = 1
			user << "<span class='notice'>Теперь используется гранатомет.</span>"
...
/obj/item/weapon/gun/projectile/automatic/proc/empty_alarm()
	if(!chambered && !get_ammo() && !alarmed)
		playsound(src.loc, 'sound/weapons/smg_empty_alarm.ogg', 40, 1, channel = "regular", time = 5)
		update_icon()
		alarmed = 1
	return
