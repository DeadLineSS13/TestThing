...
/obj/item/weapon/reagent_containers/food/drinks/attack(mob/living/carbon/human/M, mob/living/carbon/human/user, def_zone)
	if(!reagents || !reagents.total_volume)
		user << "<span class='warning'>[src] is empty!</span>"
		return 0

	if(user.a_intent == "harm")
		if(!reagents.remove_reagent("water", reagents.total_volume))
			M.wet = reagents.total_volume
			user << "<span class='warning'>You've splashed the water over the [M]!"
			return

	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		var/obj/item/organ/limb/limb = H.get_organ(def_zone)
		if(limb && limb.toxic_effects["rustpuddle"])
			if(!reagents.remove_reagent("water", 5))
				limb.toxic_effects.Remove("rustpuddle")
				user << "You've cleaned your [limb]"
				return
			else
				user << "There's not enough water to do this!"
				return
	...
	playsound(M.loc,'sound/items/drink.ogg', rand(10,50), 1, channel = "regular", time = 10)
	return 1
...
/obj/item/weapon/reagent_containers/food/drinks/soda_cans/attack(mob/M, mob/user)
	if(M == user && !src.reagents.total_volume && user.a_intent == "harm" && user.zone_selected == "head")
		...
		playsound(user.loc,'sound/weapons/pierce.ogg', rand(10,50), 1, channel = "regular", time = 10)
